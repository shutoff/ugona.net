...
public final class SpdyConnection implements Closeable {
    ...
    private final SparseArray<SpdyStream> streams = new SparseArray<SpdyStream>();
     ...
     */
    private SparseArray<Ping> pings;
    ...
    synchronized SpdyStream removeStream(int streamId) {
        SpdyStream stream = streams.get(streamId);
        if (stream != null) {
    ...
    public Ping ping() throws IOException {
        ...
        synchronized (this) {
            ...
            pingId = nextPingId;
            nextPingId += 2;
            if (pings == null) pings = new SparseArray<Ping>();
    ...
    private void close(ErrorCode connectionCode, ErrorCode streamCode) throws IOException {
        ...
        synchronized (this) {
            if (streams.size() > 0) {
                streamsToClose = new SpdyStream[streams.size()];
                for (int i = 0; i < streams.size(); i++)
                    streamsToClose[i] = streams.valueAt(i);
                streams.clear();
                setIdle(false);
            }
            if (pings != null) {
                if (pings.size() > 0) {
                    pingsToCancel = new Ping[pings.size()];
                    for (int i = 0; i < pings.size(); i++)
                        pingsToCancel[i] = pings.valueAt(i);
                }
                pings = null;
            }
        }
    ...
    class Reader extends NamedRunnable implements FrameReader.Handler {
        ...
        public void settings(boolean clearPrevious, Settings newSettings) {
            ...
            synchronized (SpdyConnection.this) {
                ...
                if (peerInitialWindowSize != -1 && peerInitialWindowSize != priorWriteWindowSize) {
                    ...
                    if (streams.size() > 0) {
                        streamsToNotify = new SpdyStream[streams.size()];
                        for (int i = 0; i < streams.size(); i++) {
                            streamsToNotify[i] = streams.valueAt(i);
                        }
                    }
                }
            }
            if (streamsToNotify != null && delta != 0) {
                for (int i = 0; i < streams.size(); i++) {
                    final SpdyStream stream = streams.valueAt(i);
                    synchronized (stream) {
        ...
        public void goAway(int lastGoodStreamId, ErrorCode errorCode, ByteString debugData) {
            ...
            synchronized (SpdyConnection.this) {
                ...
                for (int i = 0; i < streams.size(); i++) {
                    int streamId = streams.keyAt(i);
                    SpdyStream stream = streams.valueAt(i);
                    if (streamId > lastGoodStreamId && stream.isLocallyInitiated()) {
                        stream.receiveRstStream(ErrorCode.REFUSED_STREAM);
                        streams.removeAt(i);
                        i--;
                    }
                }
        ...
        public void windowUpdate(int streamId, long windowSizeIncrement) {
            if (streamId == 0) {
                ...
                final SpdyStream stream = getStream(streamId);
                if (stream != null) {
                    synchronized (stream) {
