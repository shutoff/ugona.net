...
public class HeaterFragment extends DeviceFragment {
    ...
    void fill() {
        ...
        if (rele.equals("1")) {
            if (preferences.getBoolean(Names.Car.RELE_IMPULSE + car_id, true)) {
                cmd.add(new TimerCommand(3, 0, R.string.heater_on));
            } else {
                cmd.add(new TimerCommand(5, R.drawable.icon_heater, R.string.heater_on));
                cmd.add(new TimerCommand(4, R.drawable.icon_heater_on, R.string.heater_off));
            }
        }
        if (rele.equals("2")) {
            if (preferences.getBoolean(Names.Car.RELE_IMPULSE + car_id, true)) {
                cmd.add(new TimerCommand(6, 0, R.string.heater_on));
            } else {
                cmd.add(new TimerCommand(8, R.drawable.icon_heater, R.string.heater_on));
                cmd.add(new TimerCommand(7, R.drawable.icon_heater_on, R.string.heater_off));
            }
        }
        if (rele.equals("3")) {
